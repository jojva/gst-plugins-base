...
{
  GstBuffer *buffer;
  static GstClockTime prev_ts = -40000000;
  GstClockTime diff_ts;

  buffer = GST_PAD_PROBE_INFO_BUFFER (info);
  diff_ts = GST_BUFFER_TIMESTAMP (buffer) - prev_ts;

  GST_DEBUG ("Probed 1 outbuf. ts : %" GST_TIME_FORMAT
      ", diff with previous : %" GST_TIME_FORMAT,
      GST_TIME_ARGS (GST_BUFFER_TIMESTAMP (buffer)), GST_TIME_ARGS (diff_ts));
  fail_unless_equals_uint64 (diff_ts, 40000000);
...
GST_START_TEST (test_rate)
{
  ...
  probe = gst_pad_add_probe (mysinkpad, GST_PAD_PROBE_TYPE_BUFFER,
      (GstPadProbeCallback) listen_outbuffer_ts, NULL, NULL);
  ...
  for (ts = GST_SECOND; ts < 2 * GST_SECOND; ts += GST_SECOND / 33) {
    GstBuffer *inbuf;
  ...
  assert_videorate_stats (videorate, "rate 1.0", 51, 50, 9, 9);

  /* Setting rate to 2.0 */
  g_object_set (videorate, "rate", 2.0, NULL);

  /* Push 2sec of buffers, output expected 1sec with rate 2.0 */
  for (ts = 4 * GST_SECOND; ts < 6 * GST_SECOND; ts += GST_SECOND / 33) {
    GstBuffer *inbuf;
  ...
  fail_unless_equals_int (g_list_length (buffers), 75);
  assert_videorate_stats (videorate, "rate 2.0", 118, 75, 51, 9);
